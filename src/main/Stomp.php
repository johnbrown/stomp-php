...
class Stomp {
	 ...
	 * @var boolean
	 */
	private $disconnect_on_destruct = TRUE;

	/**
	 * Constructor
	 *
	 * @param string $brokerUri Broker URL
	 * @param boolean $disconnect_on_destruct
	 * @throws StompException
	 */
	public function __construct($brokerUri, $disconnect_on_destruct = TRUE) {
		$this->_brokerUri = $brokerUri;
		$this->disconnect_on_destruct = $disconnect_on_destruct;
		$this->_init();
	}
	...
	public function subscribe($destination, $properties = null, $sync = null) {
		// This was originally set to ack "client" but the default is meant to be auto.
		// Instead, clients should specify they want the ack header to be client.
...
//        $this->_password = '';
	...
	protected function _writeFrame(StompFrame $stompFrame, $reconnect = true) {
		...
		if (($r === false || $r == 0 || feof($this->_socket)) && $reconnect) {
			$this->_reconnect();
	...
	public function readFrame() {
		...
		while (!feof($this->_socket)) {
			...
			if ($read === FALSE) {
				require_once 'Stomp/Exception.php';
				throw new StompException('connection failed while reading');
			}

			$data .= $read;

			if (ord($read) == 10 && ord($prev) == 0) {
				break;
			}
		...
		if (empty($data)) {
			require_once 'Stomp/Exception.php';
			throw new StompException('data received was empty, likely connection was lost');
		}
